...
orderFromMenu('coxinha');
  ...
  let comidas = restaurant.fetchMenu.food;
  let bebidas = restaurant.fetchMenu.drink;
  let menu = `${Object.keys(bebidas)},${Object.keys(comidas)}`.split(',');
  let menuValue = `${Object.values(bebidas)},${Object.values(comidas)}`.split(',');
  let pedido = restaurant.consumption;
  let value = 0;

  function price() {
    for (let index = 0; index < pedido.length; index += 1) {
      let position = menu.indexOf(pedido[index]);
      value += Number(menuValue[position]);
    } restaurant.pay = value * 1.1;
      return restaurant.pay;
  } 
module.exports = { createMenu, orderFromMenu, price };
