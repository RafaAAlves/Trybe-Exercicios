const {createMenu, orderFromMenu: orderFromMenu, price} = require('../src/restaurant');
...
describe('10 - Implemente a função `createMenu`, bem como seus casos de teste', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    expect(JSON.stringify(objetoRetornado)).toContain('fetchMenu');

    expect(objetoRetornado.fetchMenu).toMatchObject({ food: {}, drink: {} })

    expect(objetoRetornado.fetchMenu).toMatchObject({ food: {coxinha: 3.9, sopa: 9.9}, drink: {agua: 3.9, cerveja: 6.9}  })
    ...
    // expect(orderFromMenu('coxinha').consumption).toMatchObject(["coxinha"])
    ...
    // order('agua')
    // order('coxinha')
    // expect(order('coxinha').consumption).toMatchObject(['coxinha', 'agua', 'coxinha'])

    let objetoRetornado2 = createMenu({ food: { coxinha: 3.9, sopa: 9.9 }, drink: { agua: 3.9, cerveja: 6.9 } });
    orderFromMenu('coxinha');
    orderFromMenu('agua');
    orderFromMenu('coxinha');
    expect(objetoRetornado2.consumption).toMatchObject(["coxinha", "agua", "coxinha"])
    ...
    let objetoRetornado3 = createMenu({ food: { coxinha: 3.9, sopa: 9.9 }, drink: { agua: 3.9, cerveja: 6.9 } });
    orderFromMenu('coxinha');
    orderFromMenu('agua');
    orderFromMenu('coxinha');
    price()
    ...
    // ```
    expect(objetoRetornado3.pay).toBeCloseTo(12.87)
